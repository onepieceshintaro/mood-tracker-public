...
# 表現は3段階：
#   4日以上 下回り → 🪶 alert tone（しんどさが続いている可能性）
#   1〜3日 下回り → 📊 quiet tone（押し付けない・気にせず大丈夫）
#   0日 下回り    → 📊 quiet tone（ベースライン以上で推移）
# 「沈黙しないが警告もしない」設計。受動的可視化に留める。
try:
    ...
    if CURRENT_USER_ID and get_notify_mood_dip(CURRENT_USER_ID):
        ...
        if _n >= 3:
            ...
            if _below >= 4:
                st.caption(
                    f"🪶 直近{_n}日のうち **{_below}日** が、"
                    f"ベースライン平均（{mean_mood:.1f}）より下でした。"
                )
            elif _below > 0:
                st.caption(
                    f"📊 直近{_n}日のうち、ベースライン平均（{mean_mood:.1f}）より"
                    f"下は **{_below}日**。気にせず大丈夫そうな範囲です。"
                )
            else:
                st.caption(
                    f"📊 直近{_n}日は、ベースライン平均（{mean_mood:.1f}）以上で"
                    f"推移しています。"
                )
        else:
            st.caption(
                f"📊 気づきメモは記録が3日以上たまると表示されます"
                f"（現在 {_n}日分）。"
            )
except Exception:
    # 受動表示のため、失敗しても無視
...
if "importance" not in result:
    st.info(
        f"🌱 予測モデルは **{result['required']}日分** 溜まったら動き始める見込みです。"
        f"現在 **{result['n_train']}日**（あと {max(0, result['required'] - result['n_train'])}日くらい）。"
    )
    st.caption(
        "学習データが少ないうちは、モデルが偶然のパターンを覚えてしまうこと（過学習）があります。"
        "21日分くらい溜まると、予測値や寄与度が出てくる見込みです。"
    )
else:
    ...
    if _cv_r2 is not None and _cv_r2 < 0:
        _is_overfitted = True
        _overfit_reason = (
            f"交差検証 R² が **{_cv_r2:.2f}**（マイナス）になっています。"
            "「平均値で予測するより外しやすい」状態が見えています。"
        )
    ...
    if nd:
        ...
        if _clf_reliable:
            ...
            st.info(
                ...
                f"見られそうです（信頼度 {_conf*100:.0f}% / CV正解率 {_cv_acc*100:.0f}%）"
            )
            st.caption(
                "数値の予測はぶれが出ることもあります。"
                "**気分の流れの方向**（良くなる／同じ／下がる）は、先に見えてくることが多いです。"
            )
            ...
            if _cv_acc is not None and _baseline_acc is not None:
                st.caption(
                    f"💡 傾向予測（良くなる／同じ／下がる）も計算してみていますが、"
                    f"今のところ単純な多数派予測（{_baseline_acc*100:.0f}%）と同じくらいで、"
                    "信頼できる差がまだ見えていない状態です。"
                )

        # ----- 予測値の根拠（寄与度の上位3つを文章化） -----
        # 表現方針：「値の状態（高め/長め/遅め）」と「翌日の方向（上がる/下がる）」を分離。
        # 「遅い」など評価的な語は避け、「〜め」形で柔らかく統一する。
        _STATE_PHRASE = {
            "当日の気分": "気分が高め",
            "睡眠時間": "睡眠時間が長め",
            "エネルギー": "エネルギーが高め",
            ...
            "睡眠の前日差": "睡眠の前日差が大きめ",
            "起床の前日差": "起床の前日差が大きめ",
            "起床の7日平均からの乖離": "起床が普段より遅めにずれている",
            "直近3日の気分平均": "直近3日の気分平均が高め",
            "直近7日の気分平均": "直近7日の気分平均が高め",
        }
    ...
    with st.expander(
        ...
        st.caption(
            "プラスの棒：その特徴量が高めの日は翌日の気分が上がりやすい／"
            "マイナスの棒：高めの日は下がりやすい、とモデルが見ています。"
